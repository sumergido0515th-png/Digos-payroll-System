...
| 3 | **Postgres RLS is not available** on MySQL; Phase 2 must use the application-gateway path, which requires confining database access to a repository layer (guard now in place). | **Closed.** The gateway is `Digos\Repo\ScopeGateway` over the pure `ScopePredicate`; `DB::` is down to five grandfathered files. Deny-by-default, and a grant narrowing a dimension an entity cannot carry denies rather than widens. |
| 4 | **Tier 2 employee data is exposed to every role today** — worth fixing ahead of the full Phase 1 tier split. | **Closed.** `EmployeeSensitive` is a separate table (`0015` created and backfilled it, `0017` dropped the originals) reachable only through `employee.sensitive`. `EmployeeTierTest` asserts on the returned array *keys*, because a blanked value still travelled and a missing key never left the database. |
