...
- **~~Sixteen~~ Twelve routed endpoints no screen calls.** *(2026-09-11,
  ...
  `apiResolveDay` stays on it: the calendar screen maintains the data but nothing yet resolves a
  single date against it for one employee, which is what that endpoint answers. **Biometric import
  has no screen either**: `apiImportBiometricLogs` owns `DtrDays` and refuses to overwrite a hand-keyed day —
