...
- **Sixteen routed endpoints no screen calls, and one of them is a whole entity.** *(2026-09-11,
  ...
  built**, and two are worth scheduling. **Holidays have no screen at all**: migration `0019`
  shipped `Holidays` and `HolidayPayRules`, `apiListHolidays`/`apiSaveHoliday`/`apiDeleteHoliday`/
  `apiListHolidayPayRules` are routed and permissioned on `calendar.view`/`calendar.edit`, and the
  only trace in the whole UI is a legend label in `views/coverage.php` and a `Holiday` day-type
  option in `views/dtr.php` — so nobody can enter a holiday, and the pay rules that key off one
  cannot fire. `apiResolveDay` is in the same cluster, and `app/Calendar.php`'s own header calls it
  "the endpoint a screen calls to ask what this date was". **Biometric import has no screen
  either**: `apiImportBiometricLogs` owns `DtrDays` and refuses to overwrite a hand-keyed day —
