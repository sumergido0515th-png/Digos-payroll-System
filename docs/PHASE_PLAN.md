...
**Status:** IN PROGRESS — 9A landed 2026-08-31
**Depends on:** Phases 1–8 stable

> **9A landed: the query core, proven on Payroll end to end.** `FilterSpec` and
> `FilterSql` (`app/Domain/Query/`, both pure) replace the ad hoc per-column
> loop `PayrollRepo::listScoped()` used to run inline, the same shape the
> "Implementation plan" below flags as unable to carry faceted filters without
> risking a leak. `FilterSpec` is a hardcoded facet allowlist mapping a name to
> a real column — exactly `ScopeEntity`'s role for scope dimensions, and for
> the same reason: a column name is safe to interpolate only because it never
> comes from the payload. `FilterSql` turns a normalized request into a bound
> fragment and never sees `$user`, composed by the repo as
> `WHERE (scope) AND (filters)`.
>
> **`listScoped()` was rewritten in place, not given a parallel `search()`
> method.** It has three callers, not one — `apiListPayrolls`,
> `apiGetWorklist`, and `apiRunReport` — found by grepping before designing
> rather than assumed from the addendum below, and all three now run through
> the hardened layer with no call-site changes.
>
> Payroll's facets are the four columns `listScoped()` already filtered on
> (`PeriodID`, `OfficeCode`, `Department`, `Status`) plus `PreparedByUser`;
> `FunctionCode` is deliberately still absent — see this phase's own
> "Carried-over risk" note below, unchanged by this session. No pagination was
> added: the task list never names it, the exit gate is a disclosure test that
> row count doesn't strengthen, and `listScoped()`'s other two callers need
> the complete scoped set for correct counts, so a shared `LIMIT` would
> silently produce a wrong report total.
>
> **The exit-gate test is `tests/Integration/PayrollFilterDisclosureTest.php`,
> and it was verified to fail, not just written to pass.** Injecting the
> plausible bug this layer exists to prevent — joining scope and filters with
> `OR` instead of `AND` — failed 4 of its 6 tests (the two controls correctly
> stayed green), then the fix was restored and the full suite re-confirmed at
> 493. Two of the addendum's three disclosure vectors apply to 9A and both are
> covered: filtering explicitly for the out-of-scope office, and a search term
> matching only the out-of-scope row, both return empty rather than a refusal.
> **Vector 2 (facet options themselves scoped) is deliberately NOT covered
> here** — `apiGetLookups()` in `app/Master.php` returns `Offices` via a bare
> `DB::rows()` with no scope predicate at all, a real and pre-existing gap,
> but it is a shared endpoint feeding every module's dropdowns, not something
> 9A's new files touch, and the addendum assigns "scoping the facet options"
> to 9B by name. Vector 3 (export parity) does not apply until 9D, since no
> export path exists yet.
>
> No migration, no new permission, and no `DatabaseAccessTest`/`RouteTableTest`
> change were needed. Suite is 493 tests (up from 471).
...
| 2026-08-31 | **Phase 9A landed: FilterSpec/FilterSql, and PayrollRepo::listScoped() rewritten to compose them with ScopeGateway rather than its own ad hoc per-column loop.** Full detail is in Phase 9's own status block above rather than repeated here. Worth surfacing in this log specifically: three callers of `listScoped()` were found by grepping (`apiListPayrolls`, `apiGetWorklist`, `apiRunReport`), not the one the addendum's own worked example implied, which is why the method was rewritten in place rather than given a parallel `search()` name — a second name doing the same query would drift the moment 9B added a facet to only one of them. The exit-gate test was verified to fail before being trusted: joining scope and filters with `OR` instead of `AND` failed 4 of its 6 assertions, the fix was restored, and the full suite re-confirmed green. Suite is 493 tests.
