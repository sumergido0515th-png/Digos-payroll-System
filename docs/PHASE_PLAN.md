...
**Status:** IN PROGRESS — 9A, 9B and 9C done, 9D–9E outstanding
...
### What landed in 9C (2026-08-29)

**The four watchlists, as saved filters rather than queries of their own.**
`Digos\Domain\Query\Watchlist` is pure and takes today as an argument; it turns a date into a
payload `FilterSpec` already understands, and the ordinary scoped `search()` answers it. Four
endpoints, each routed on the permission its entity already uses:
`apiGetExpiringBioExemptions`, `apiGetExpiringContracts`, `apiGetStaleMemoranda`,
`apiGetOverdueSuspensions`.

**Why not a query each.** A bespoke SQL query per watchlist is the shape of the July
`printBundle()` leak — a second path to the same rows, built beside the scoped one rather than on
top of it. Expressing each as a payload means an office-scoped user's watchlist is their own
office's records with no scope code written here at all, and the same four are URL-encodable (9E)
and exportable (9D) for free.

Two ops were added to the core and no more: `before` (strictly earlier — "untouched since March"
means the last edit was *before* March) and `isNull`, the one condition that binds no value. The
column it may ask about still comes from `FilterSpec`; only the direction comes from the payload.

**Every threshold is a decision, and each one is now written down and tested:**

| Watchlist | Bounded | Why |
|---|---|---|
| Bio exemptions | `ValidTo` between today and today+15 | Bounded at **both** ends: one that lapsed last month has already expired — a different problem with a different remedy — and listing it would bury the ones still worth renewing |
| Contracts | `EndDate <= periodEnd`, **no lower bound** | The opposite choice, deliberately: a contract that ended before the period began is the *worse* case, not an old record to skip |
| Memoranda | `EffectivityType = 'OpenEnded'` + not revoked + `UpdatedAt` older than 6 months | See below |
| Suspensions | `Deadline < today`, Open only | Strictly before: one due *today* is not yet overdue, and saying so on the morning of the deadline is how a list stops being read |

**Decision 3 was right to have been questioned.** The predicate this plan originally guessed —
`EffectivityEnd IS NULL` — would have shipped a watchlist that also reported every `Specific` and
`Recurring` memo, both of which legitimately carry no end date and neither of which is open-ended.
The fixture carries one of each, stale, precisely so that mistake cannot come back.

The period is **required** for the contracts watchlist rather than defaulting to today: "expiring
before period end" is a question about a specific period, and quietly substituting today would
answer a different question that looks identical on screen.

**Exit gate.** `tests/Unit/WatchlistTest.php` pins the boundary arithmetic to exact dates, and
`tests/Integration/WatchlistScopeTest.php` puts a record just inside and just outside every
threshold, then asserts both the right rows and that no watchlist discloses another office.
Verified by sabotaging three predicates — the open-ended type, the revoked exclusion, and the
overdue boundary — which produced 8 failures across both files.

One quirk recorded rather than fixed: PHP's month arithmetic overflows a short month, so six
months back from 31 August is 3 March rather than 28 February. Immaterial for a housekeeping
horizon, and left alone — but written down with a test, because the same arithmetic *would* matter
somewhere a deadline is legally meaningful.
...
| 2026-08-29 | **Phase 9C: the four watchlists, as saved filters rather than queries of their own.** `Watchlist` is pure and takes today as an argument, turning a date into a payload `FilterSpec` already understands so the ordinary scoped `search()` answers it — which is what makes an office-scoped user's watchlist their own office's records with no scope code written in this session at all, and what keeps these off the second-path-to-the-same-rows shape that produced the July print leak. Four endpoints, each on the permission its entity already uses. Two ops added to the core and no more: `before`, strictly earlier, because "untouched since March" means the last edit was *before* March; and `isNull`, the one condition binding no value — the column it may ask about still comes from `FilterSpec` and only the direction comes from the payload. **Every threshold is a decision and each is now written down and tested**, including two that point opposite ways on purpose: the exemptions watchlist is bounded at both ends, because one that lapsed last month has already expired and listing it would bury the ones still worth renewing, while the contracts watchlist has no lower bound at all, because a contract that ended before the period began is the *worse* case rather than an old record to skip. Suspensions are strictly past their deadline — one due today is not yet overdue, and saying so on the morning of the deadline is how a list stops being read. **Decision 3 was right to have been questioned**: the predicate this plan originally guessed, `EffectivityEnd IS NULL`, would have shipped a watchlist that also reported every `Specific` and `Recurring` memo, both of which legitimately carry no end date and neither of which is open-ended; the fixture now carries one of each, stale, so that mistake cannot come back. The period is required for the contracts watchlist rather than defaulted to today, because "expiring before period end" is a question about a specific period and substituting today would answer a different one that looks identical on screen. Exit gate is a unit file pinning the boundary arithmetic to exact dates plus an integration file placing a record just inside and just outside every threshold; verified by sabotaging the open-ended type, the revoked exclusion and the overdue boundary, which produced 8 failures across both. One quirk recorded rather than fixed: PHP's month arithmetic overflows a short month, so six months back from 31 August is 3 March — immaterial for a housekeeping horizon, but written down with a test because the same arithmetic would matter where a deadline is legally meaningful. Suite is 580 tests |
