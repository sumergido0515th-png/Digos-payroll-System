...
| `.htaccess` honoured (`AllowOverride All`) | **The only thing keeping `app/`, `migrations/`, `backups/` and `attachments/` off the public web** |
...
htdocs/
  ...
  attachments/    DENIED   <- uploaded scans, medical certificates
...
5. Scroll down and click **Go**. Wait for it to finish — this is one file creating
   31 tables, so it can take a little while on a shared server. Don't close the tab.
...
You could delete the leftover tables by hand instead, but there are forty-three foreign
...
drops all thirty-one tables in one paste. It's generated by
...
6. **Turn on "show hidden files"** in FileZilla first (Server menu →
   *Force showing hidden files*) — otherwise it won't show or upload `.htaccess`,
   and without those files `app/`, `backups/` and `attachments/` are readable by
   anyone with the URL.
...
| `https://<site>/attachments/` | **403**, no listing — uploaded scans and medical certificates live here |
